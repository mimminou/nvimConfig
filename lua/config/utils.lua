...
function GFun.show_git_blame()
	...
	local commit, author, date, summary = "?", "Unknown", "?", ""
	...
	for _, l in ipairs(output) do
		...
		local t = l:match("^author%-time%s+(%d+)")
		if t then date = os.date("%Y-%m-%d %H:%M", tonumber(t)) end
		local s = l:match("^summary%s+(.+)")
		if s then summary = s end
	end

	local display_commit = (commit == "uncommitted") and "" or commit:sub(1, 8)

	local text = author .. " • " .. date
	if display_commit ~= "" then
		text = text .. " • " .. display_commit
	end
	text = text .. " • " .. summary
